...
-- Executa os turnos.
turnos :: Personagem -> Personagem -> Int -> (Personagem, Personagem, [String])
turnos input1 input2 rodada =
    let 
        ...
        str1 = "No turno " ++ show (rodada + 1) ++ ", " ++ (nome p1) ++ " atacou " ++ (nome p2) ++ " causando " ++ show dano1 ++ " de dano."
        tem2turnos = (hp p2att) > 0
        dano2 = calculoDano (forca p2att) (defesa p1)
        p1att = p1 {hp = hp p1 - dano2}
        str2 = if tem2turnos 
            then "No turno " ++ show (rodada + 2) ++ ", " ++ (nome p2) ++ " atacou " ++ (nome p1) ++ " causando " ++ show dano2 ++ " de dano!!"
            else (nome p2) ++ " morreu!!"
...
combate p1 p2 turnoAtual = if alguemMorreu then finalizado else combate p1att p2att logAcumulado
        where
            ...
            (p1att, p2att, str) = turnos p1 p2 (rodadasLevadas turnoAtual*2)
