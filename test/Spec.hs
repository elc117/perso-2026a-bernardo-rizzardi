...
testes = TestList
    ...
    , TestCase (let (_, _, log) = turnos p1 p2
                in assertEqual "Executa turno entre p1 e p2" "No turno 1, Aragorn atacou Goblin causando 15 de dano. No turno 2, Goblin atacou Aragorn causando 1 de dano!!" log)
    , TestCase (let
                    resultInic = resultadoInicial
                    relatorio = combate p1 p2 resultInic
                    winner = vencedor relatorio
                in assertEqual "Verifica vencedor entre p1 e p2" "Aragorn" winner)
    ]